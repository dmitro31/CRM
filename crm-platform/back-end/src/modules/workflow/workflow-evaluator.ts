...
export class WorkflowEvaluator {
  static matchesTrigger(
    ...
    const triggerEvent = trigger.event as WorkflowEvent;

    if (triggerEvent === WorkflowEvent.RECORD_CREATED) {
      return event === 'RECORD_CREATED';
    }

    if (triggerEvent === WorkflowEvent.RECORD_UPDATED) {
      return event === 'RECORD_UPDATED';
    }

    if (triggerEvent === WorkflowEvent.FIELD_CHANGED) {
