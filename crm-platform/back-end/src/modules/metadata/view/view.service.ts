...
export class ViewService {
  ...
  async create(moduleId: string, userId: string, dto: CreateViewDto) {
  ...
  async findOne(viewId: string, userId: string) {
    const view = await this.findViewOrThrow(viewId);

    await this.workspaceAccess.ensureModuleAccess(view.moduleId, userId);

    return view;
  }

  async update(viewId: string, userId: string, dto: UpdateViewDto) {
    const view = await this.findViewOrThrow(viewId);
  ...
  async remove(viewId: string, userId: string) {
    const view = await this.findViewOrThrow(viewId);
