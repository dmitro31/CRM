...
Питання користувача: "${dto.question}"`
    ...
    const answer = await this.ai.chatWithTools(
      prompt,
      TOOLS,
      (name, args) => this.executeTool(workspaceId, userId, name, args),
    )
  ...
  private async executeTool(
    ...
    if (name === 'search_records') {
      return this.searchRecords(workspaceId, userId, args)
    }

    if (name === 'count_records') {
      return this.countRecords(workspaceId, userId, args)
    }
  ...
  private async searchRecords(
    workspaceId: string,
    userId: string,
    args: { module_name: string; filters?: Record<string, string>; limit?: number },
  ) {
