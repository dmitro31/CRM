...
export class AuthService {
  ...
  async logout(refreshToken: string) {
    ...
    await this.refreshTokenService.revokeSession(payload.tid);
  ...
  async forgotPassword(email: string) {
    ...
    const token = await this.verificationTokenService.createPasswordResetToken(
      user.id,
    );
  ...
  async googleLogin(profile: {
    ...
    if (!user) {
      ...
      if (user.provider === 'LOCAL' || !user.avatar) {
    ...
    if (!user) {
      ...
      if (user.provider === 'LOCAL' || !user.avatar) {
        user = await this.prisma.user.update({
          ...
          data: {
            provider: user.provider === 'LOCAL' ? 'GITHUB' : user.provider,
            avatar: user.avatar ?? profile.avatar,
          },
        });
      }
